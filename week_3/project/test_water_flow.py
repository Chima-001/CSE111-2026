# I added a test function for the convert_kpa_to_psi function and called it multiple times (lines 46-51)
...
def test_pressure_gain_from_water_height():
    assert pressure_gain_from_water_height(0.0) == approx(0.000, abs= 0.01)
...
def test_convert_kpa_to_psi():
    ...
    assert convert_kpa_to_psi(62.1) == approx(9.01, abs= 0.01)
    assert convert_kpa_to_psi(158.7) == approx(23.02, abs= 0.01)
    assert convert_kpa_to_psi(250) == approx(36.259, abs= 0.01)
    assert convert_kpa_to_psi(500) == approx(72.52, abs= 0.01)
